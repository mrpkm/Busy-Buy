...
const CostumeItemContext = ({ children }) => {
    ...
    useEffect(() => {
        ...
        // price range
        const rangePricess = searchfilter.filter((item) => item.price <= rangePrice);
        // check box 

        const filteredByCategories = checkBox.length === 0
            ? rangePricess
            : rangePricess.filter((item) =>
                checkBox.includes(item.category)
            );
    ...
    useEffect(() => {
        onAuthStateChanged(auth, (user) => {
            if (user) {
                const unsubscribe = onSnapshot(query(collection(db, "users", user.uid, "carts"), orderBy("id", "desc")), (snapshot) => {
    ...
    return (
        <itemContext.Provider value={{ data, addToCart, cart, removeToCart, increaseQty, decreaseQuantity, buyNow, totalPrice, allOrder, loding, searchQuery, handleSearach, filterData, rangePrice, handleRange, filterCategory, checkBox, handleCategory, redTotal }}>
            {children}
